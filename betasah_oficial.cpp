//prof. Carmen Minca, Colegiul National de Informatica Tudor Vianu Bucuresti
#include <fstream>
using namespace std;

int main()
{int n,d,k,pozdama[103][3], albe[1003], P=0, M;
char a[1003][1003];
...
ofstream g("betasah.out");
    ...
    int i,l,c,j,i1;
    f>>n>>d>>k;
    //initializez partea inferioara a matricei cu 0
    for(i=1;i<=n;i++)
    ...
    for(i=1;i<=d;i++)
    {
        ...
        //pe diagonala 2-6
        i1=l-1; j=c+1;
            while (a[i1][j]!=2)//directia 2
            { if(a[i1][j]!=1) a[i1][j]=3;
              i1--; j++;}
            i1=l+1; j=c-1;
            while (a[i1][j]!=2)//directia 6
            { if(a[i1][j]!=1) a[i1][j]=3;
              i1++; j--;}
    }
